"""
UCD Verification — Streamlit UI page.

This is the main page that users interact with to upload TTool XML files
and verify Use Case Diagrams. Extracted from the old ``ucd_validation.py``.
"""
...
from verification.ttool_launcher import launch_ttool_app, get_file_hash
    ...
    if uploaded_file is not None:
        ...
        if validated_count == total_ucd_count:
            if not st.session_state["finalized"]:
                if st.button("Finalize & Apply All Verified Classifications"):
                    ...
                    steps = element_verification.collect_issue_steps(st.session_state["ucd_dataset"])
                    raw_xml = st.session_state.get("uploaded_xml", "")
                    if raw_xml:
                        xml_with_steps = element_verification.append_activity_panel(raw_xml, steps)
